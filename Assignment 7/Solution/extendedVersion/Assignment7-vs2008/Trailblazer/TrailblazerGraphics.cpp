...
const string kRandomMazeLabel("Random Maze(Kruskal)		 ");
const string kRandomMazeLabel1("Random Maze(Prim)		 ");
...
gTypeList->addItem(kRandomMazeLabel1);
...
static bool regenerateWorld(Grid<double>& world, WorldType& worldType) {
  ...
  if (typeLabel == kRandomTerrainLabel) {
    ...
    try {
       ...
       */
      newWorld = generateRandomMazeKruskal(numRows / 2 + 1, numCols / 2 + 1);
      newType = MAZE_WORLD;
	...
	else if (typeLabel == kRandomMazeLabel1) {
    int numRows = kMazeNumRows[worldSize];
    int numCols = kMazeNumCols[worldSize];
    ...
    try {
       ...
       * maze based on the number of logical rows and columns we want.
       */
      newWorld = generateRandomMazePrim(numRows / 2 + 1, numCols / 2 + 1);
      newType = MAZE_WORLD;
    } catch (const ErrorException& e) {
      cout << e.getMessage() << endl;
      return false;
